...
fn test_minidb_with_ipc_insert() {
    ...
    db2.insert(&mut r).expect("failed to insert restaurant");

    let mut o = Order {
        id: String::new(),
        restaurant_id: r.id,
    };
    db2.insert(&mut o).expect("failed to insert order");

    let all_orders = db2.all::<Order>().expect("failed to get all orders");
    assert_eq!(all_orders.len(), 1);

    let all_restaurants = db2
        .all::<Restaurant>()
...
#[test]
fn test_minidb_with_ipc_insert_many() {
    const N: usize = 1000;
    ...
    db2.insert_many(&mut restaurants)
...
#[test]
fn test_minidb_with_ipc_update_many() {
    const N: usize = 1000;
...
fn test_minidb_with_ipc_get_non_existent() {
    const N: usize = 1000;
...
#[test]
fn test_minidb_with_ipc_remove() {
    const N: usize = 1000;
...
#[test]
fn test_minidb_with_ipc_remove_many() {
    const N: usize = 1000;
...
#[test]
fn test_minidb_with_ipc_for_each() {
    const N: usize = 1000;
...
#[test]
fn test_minidb_with_ipc_export_table() {
    const N: usize = 1000;
